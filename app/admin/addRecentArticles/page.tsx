...
export default function AddRecentArticlesPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post("/api/RecentArticles", formData);

      if (res.data.success) {
        toast.success("Recent Articles item added!");
        setFile(null);
        setText("");
        setPreviewUrl(null);
      }
  ...
  return (
    <>
      <form onSubmit={onSubmit} className="pt-8 px-6 max-w-xl">
        <h2 className="text-xl font-semibold mb-4">Add Recent Articles</h2>
